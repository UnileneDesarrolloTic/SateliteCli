...
export class ControlProductoTerminadoComponent implements OnInit {
  ...
  PromedioResistencia2(index: number) {
    ...
    this.FormProtocolo.get("CampoPromvacio2RCP").patchValue(this._GenericoService.NumberTwoDecimal(Min, 4));
  ...
  PromedioUnion2(index:number){
    ...
    this.FormProtocolo.get("CampoPromvacio3RCP").patchValue(this._GenericoService.NumberTwoDecimal(Min, 4));
    ...
    D2=D1/(cn-1);
    desv= Math.pow(D2,0.5);
    this.FormProtocolo.get("CampoPromvacio5RCP").patchValue(this._GenericoService.NumberTwoDecimal(desv,4));
