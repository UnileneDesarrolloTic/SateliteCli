...
@Component({
  selector: 'app-tag-control-proceso',
  templateUrl: './tag-control-proceso.component.html',
  styleUrls: ['./tag-control-proceso.component.css']
})
export class TagControlProcesoComponent implements OnInit {
  ...
  ConstruirTabla1(Tabla) {
    const ArrayItem = this.Form.controls.TablaLongitudCP as FormArray;
    ArrayItem.controls = [];
  ...
  ConstruirTabla2(Tabla) {
    const ArrayItem = this.Form.controls.TablaResistenciaCP as FormArray;
    ArrayItem.controls = [];
    Tabla.forEach((itemRow: any) => {
      const ItemFilaForm = this._fb.group({
        TensionNewtons: [],
  ...
  get ListTabla1() {
    return this.Form.controls['TablaLongitudCP'] as FormArray;
  }

  get ListTabla2() {
    return this.Form.controls['TablaResistenciaCP'] as FormArray;
  }

  //PRIMERA TABLA
  PromedioLongitud2(index: number) {
    // const myForm = (<FormArray>this.Form.get("TablaLongitudCP")).at(index);
    // console.log(myForm)
    const myForm = this.Form.controls.TablaLongitudCP.value;
    let PL_Suma = 0;
    let cn = 0;
    let promedio = 0;
    let D1 = 0;
    ...
    promedio = (PL_Suma / cn);
    this.Form.get("CampoPromvacioCP").patchValue(promedio);
    ...
    this.Form.get("CampoPromvacio2CP").patchValue(this._GenericoService.NumberTwoDecimal((isNaN(desv) ? 0 : desv), 4));
  ...
  PromedioDiametro2(index) {
    ...
    this.Form.get("CampoPromvacio1CP").patchValue(promedio);
    ...
    desv = Math.pow(D2, 0.5);
    // console.log(desv);
    this.Form.get("CampoPromvacio3CP").patchValue(this._GenericoService.NumberTwoDecimal((isNaN(desv) ? 0 : desv), 4));
  ...
  PromedioResistencia2(index: number) {
    ...
    const myForm = this.Form.controls.TablaResistenciaCP.value;
    let PL_Suma = 0;
    let cn = 0;
    let promedio = 0;
    var D1 = 0;
    ...
    this.Form.get("CampoPromvacioRCP").patchValue(promedio);
    myForm.forEach((element: any) => {
    ...
    this.Form.get("CampoPromvacio2RCP").patchValue(this._GenericoService.NumberTwoDecimal(Min, 4));
    ...
    D2 = D1 / (cn - 1);
    desv = Math.pow(D2, 0.5);
    this.Form.get("CampoPromvacio4CP").patchValue(this._GenericoService.NumberTwoDecimal((isNaN(desv) ? 0 : desv), 4));
  ...
  PromedioUnion2(index:number){
    ...
    this.Form.get("CampoPromvacio1RCP").patchValue(promedio);
    ...
    this.Form.get("CampoPromvacio3RCP").patchValue(this._GenericoService.NumberTwoDecimal(Min, 4));
    ...
    D2=D1/(cn-1);
    desv= Math.pow(D2,0.5);
    this.Form.get("CampoPromvacio5RCP").patchValue(this._GenericoService.NumberTwoDecimal(desv,4));
