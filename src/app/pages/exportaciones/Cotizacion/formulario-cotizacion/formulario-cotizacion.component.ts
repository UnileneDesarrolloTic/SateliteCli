...
import { ModalItemMastComponent } from '@shared/components/modal-item-mast/modal-item-mast.component';
...
export class FormularioCotizacionComponent implements OnInit {
  ...
  DetalleCotizacion(Detalle: DetalleCotizacionExportaciones[]) {
    ...
    Detalle.forEach((itemRow: DetalleCotizacionExportaciones) => {
      const ItemFilaForm = this._fb.group({
        ...
        linea: [this.FormularioNuevo ? itemRow.linea : 0, Validators.required],
        ...
        materiaprima: [itemRow.materiaprima, Validators.required],
        receta: [itemRow.receta, Validators.required],
        cantidadpedida: [itemRow.cantidadpedida, Validators.required],
      });
  ...
  Guardar() {
    ...
    if (this.DetalleCoti.length > 0) {
      if (this.formFormulario.valid == true) {
        this._exportacionesService.GuardarCotizacionExportaciones(dato).subscribe(
          (resp) => {
            if (resp["success"]) {
              this._toastrService.success(resp["content"]);
              this._router.navigate(['Exportaciones', 'Cotizacion', 'listar'])
            } else {
        ...
        this._toastrService.warning("Debe completar los datos de la cotización");
      }
    } else {
  ...
  AbrirModalMP(item: DetalleCotizacionExportaciones) {

    const modalRefDetalleReceta = this._modalService.open(ModalDetalleRecetaComponent, {
      ariaLabelledBy: 'modal-basic-title',
      centered: true,
    ...
    modalRefDetalleReceta.componentInstance.Item = item.item;
  ...
  EliminarItem(itemRow: DetalleCotizacionExportaciones, index) {
    if (this.FormularioNuevo == false) {
      this._toastrService.success("Eliminación exitosa");
      ...
      if (itemRow.linea == 0) {
        ...
        this._exportacionesService.DesactivarItemCotizacionExportacion(this.formFormulario.controls.NumeroDocumento.value, itemRow.item, itemRow.linea).subscribe(
          (resp: any) => {
            if (resp["success"]) {
              this._toastrService.success(resp["content"]);
              this.formarrayDetalle().removeAt(index);
          ...
          (error) => {
            this._toastrService.info("Comuniquese con sistema")
          }
        )
      }
    }
  ...
  AgregarItem() {
    const modalAgregarItem = this._modalService.open(ModalItemMastComponent, {
      ariaLabelledBy: "modal-basic-title",
      backdrop: "static",
      size: "lg",
    });
    ...
    modalAgregarItem.componentInstance.fromParent = '';
    modalAgregarItem.result.then((ArrayItem) => {

      ArrayItem.forEach((element: DetalleCotizacionExportaciones) => this.Agregar(element))
      this.formFormulario.get("AjusteRedondeo").valueChanges.pipe(debounceTime(500)).subscribe(resp => this.sumarMonto());
    }, (reason) => {
  ...
  Agregar(element: DetalleCotizacionExportaciones) {
    const ItemFilaForm = this._fb.group({
      cantidad: [element.cantidad, Validators.required],
      linea: [0, Validators.required],
      codSut: [element.codSut, Validators.required],
      descripcion: [element.descripcion, Validators.required],
      iGVExoneradoFlag: [element.iGVExoneradoFlag, Validators.required],
      item: [element.item, Validators.required],
      monto: [element.monto, Validators.required],
      precioUnitario: [element.precioUnitario, Validators.required],
      materiaprima: [element.materiaprima, Validators.required],
      receta: [element.receta, Validators.required],
      cantidadpedida: [element.cantidadpedida, Validators.required],
    });
